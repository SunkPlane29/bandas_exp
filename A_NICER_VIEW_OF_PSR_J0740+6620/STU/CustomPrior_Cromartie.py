...
_mass_mu = 2.14; _mass_std = 0.1
_sin_i_mu = 0.9990; _sin_i_std = 0.0002
...
def distance_PDF(D):
    ...
    F_D *= np.exp(-0.5 * ((1.0/D - 0.5)/0.3)**2.0)

    return D * D * F_D

distances = np.linspace(0.5, 1.7, 1000)
CDF = np.zeros(1000)
for i in range(len(distances)):
    CDF[i] = quad(distance_PDF, 0.5, distances[i])[0]
    ...
    * p[14] = energy-independent XMM effective-area scaling (relative to XTI)
    ...
    def inverse_sample(self, hypercube=None):
        ...
        ref['mass'] = truncnorm.ppf(hypercube[idx], -5.0, 5.0,
                                    loc=_mass_mu, scale=_mass_std)

        idx = ref.index('cos_inclination')
        _sin_i = truncnorm.ppf(hypercube[idx], -5.0, 5.0,
                               loc=_sin_i_mu, scale=_sin_i_std)
        ref['cos_inclination'] = math.sqrt(1.0 - _sin_i * _sin_i)
