    ...
    * p[14] = energy-independent XMM effective-area scaling (relative to XTI)
    ...
    def density(self, point):
        ...
        try:
            ...
            _a = ((ref['XTI__alpha'] - 1.0)/0.15)
            _b = ((ref['PN__alpha'] - 1.0)/0.15)
            _exponent = _a * _a
            _exponent += _b * _b
            _exponent -= 2.0 * 0.5 * _a * _b
            _exponent /= 2.0 * (1.0 - 0.5*0.5)

            density *= math.exp(-1.0 * _exponent)
            density /= 2.0 * math.pi * 0.15 * 0.15 * math.sqrt(1.0 - 0.5 * 0.5)
